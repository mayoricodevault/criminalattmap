...
add_filter( 'parent_file', 'va_event_forms_tax_menu_fix', 0, 20 );
...
function va_event_forms_tax_menu_fix( $parent_file ) {
	global $submenu;

	$event_tax = get_taxonomy( VA_EVENT_CATEGORY );

	if ( isset( $submenu['edit.php?post_type=custom-form'] ) ) {
		foreach( $submenu['edit.php?post_type=custom-form'] as $k => $submenu_item ) {
			if ( $submenu_item[0] == $event_tax->labels->menu_name )
