...
class VA_Widget_Event_Attendees extends WP_Widget {
	...
	function create_attendance_list( $event_id ) {
		...
		foreach ( $users as $user ) {
			$user_url = get_author_posts_url( $user->ID, $user->user_nicename );
			...
			$a_name = html( 'a', array( 'href' => $user_url, 'class' => 'name' ), $user->user_nicename );
...
class VA_Widget_Create_Event_Button extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( is_tax( VA_EVENT_CATEGORY ) ) {
			$url = add_query_arg( VA_EVENT_CATEGORY, get_queried_object_id(), $url );
		}
